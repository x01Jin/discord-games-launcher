...
class ProcessManager:
    ...
    def _verify_game_process(self, game_id: int, pid: int) -> bool:
        ...
        try:
            ...
            # Match an exact path segment: a substring check confuses
            # game 22 with .../222/... after PID recycling.
            return str(game_id) in Path(exe_path).parts
    ...
    def reset_runtime_state(self) -> int:
        """Drop all runtime tracking without touching OS processes.

        Startup-only: PIDs recorded by a previous run are meaningless after
        a restart (and may have been recycled by unrelated processes), so
        records are cleared instead of stopped.

        Returns:
            Number of stale records cleared.
        """
        running = self.db.get_running_processes()
        for game_id in running:
            try:
                ...
                if self.logger:
                    self.logger.warning(
                        f"Could not clear runtime record for game {game_id}: {e}"
                    )
                continue
        self._local_pid_cache.clear()
        return len(running)
